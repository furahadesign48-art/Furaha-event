...
const Dashboard = ({ selectedTemplate, userData: propUserData, onLogout, onBackToHome }: DashboardProps) => {
  ...
  // Compteur d'activités pour déclencher la modal d'avis
  const [showTestimonialModal, setShowTestimonialModal] = useState(false);
  const TESTIMONIAL_TRIGGER_ACTIONS = 7;
  const activityCountRef = React.useRef<number>(0);
  const activityCountLoadedRef = React.useRef<boolean>(false);
  const canShowTestimonialRef = React.useRef<boolean>(true);

  const getStorageKey = (suffix: string) =>
    `furaha:testimonial:${userData?.uid || 'guest'}:${suffix}`;

  // Charger le compteur + l'état déjà vu au montage
  useEffect(() => {
    if (!userData?.uid) return;
    try {
      const storedCount = parseInt(localStorage.getItem(getStorageKey('count')) || '0', 10);
      const storedSubmitted = localStorage.getItem(getStorageKey('submitted')) === '1';
      const storedDismissed = localStorage.getItem(getStorageKey('dismissed')) === '1';
      activityCountRef.current = isFinite(storedCount) ? storedCount : 0;
      canShowTestimonialRef.current = !storedSubmitted && !storedDismissed;
    } catch {
      activityCountRef.current = 0;
    }
    activityCountLoadedRef.current = true;
  }, [userData?.uid]);

  const incrementActivity = React.useCallback(() => {
    if (!userData?.uid || !activityCountLoadedRef.current) return;
    activityCountRef.current += 1;
    try {
      localStorage.setItem(getStorageKey('count'), String(activityCountRef.current));
    } catch {}
    if (
      canShowTestimonialRef.current &&
      activityCountRef.current >= TESTIMONIAL_TRIGGER_ACTIONS
    ) {
      canShowTestimonialRef.current = false;
      // Petit délai pour ne pas gêner l'action en cours
      const t = window.setTimeout(() => setShowTestimonialModal(true), 900);
  ...
  const handleTestimonialClose = () => {
    ...
    try {
      localStorage.setItem(getStorageKey('dismissed'), '1');
    } catch {}
  };

  const handleTestimonialSubmitted = () => {
    try {
      localStorage.setItem(getStorageKey('submitted'), '1');
    } catch {}
  };
  ...
  const handleEditTemplate = (template: TemplateData) => {
    // Fusionner avec userModels[] correspondant par ID pour récupérer customizations
    // (important quand selectedTemplate vient de WeddingTemplate sans customizations)
    const fullModel = template.id ? userModels.find(m => m.id === template.id) : undefined;
    const merged: any = fullModel
      ? { ...template, ...(fullModel as any), id: fullModel.id,
          colors: (fullModel as any).colors || (template as any).colors,
          customizations: (fullModel as any).customizations || (template as any).customizations }
      : template;
    setEditingTemplate(merged as TemplateData);
  };
  ...
  const handleSaveTemplate = async (customizedTemplate: TemplateData) => {
    ...
    try {
      ...
      // Propager les customizations (layout, couleurs, fonts) depuis TemplateCustomization
      const anyTemplate = customizedTemplate as any;
      if (anyTemplate.customizations && typeof anyTemplate.customizations === 'object') {
        (updates as any).customizations = anyTemplate.customizations;
      }
      if ((customizedTemplate as any).colors) {
        (updates as any).colors = (customizedTemplate as any).colors;
      }
...
const renderOverview = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f17] relative overflow-hidden">
      ...
      {/* Modal d'avis client (déclenchement automatique) */}
      <TestimonialModal
