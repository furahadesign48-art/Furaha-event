...
type SvgState = { 
 ...
 export const LiquidButton: React.FC<LiquidButtonProps> = React.memo(({ onClick, isConfirmed, colors, text }) => {
    const [isHovered, setIsHovered] = useState(false);
    return (
        <div className="flex justify-center w-full max-w-xs mx-auto"> 
          <div 
            className="relative inline-block w-full h-[3.5em] group dark:bg-black bg-white dark:border-white border-black border-2 rounded-full overflow-hidden"
          ...
          > 
            ...
            <button 
              className="absolute inset-0 rounded-full bg-transparent cursor-pointer flex items-center justify-center gap-3 px-6 z-10" 
              onClick={onClick}
              aria-label={text} 
              type="button" 
            > 
              <span className="flex items-center gap-3 text-white text-lg font-bold tracking-wider whitespace-nowrap"> 
                {isConfirmed ? (
