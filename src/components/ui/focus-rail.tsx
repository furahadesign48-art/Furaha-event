"use client"; 
 ...
 export const FocusRail = React.memo(({ 
   items, 
   initialIndex = 0, 
   loop = true, 
   autoPlay = false, 
   interval = 4000, 
   className, 
   accentColor = "#10B981", // Emerald-500 par défaut
 }: FocusRailProps) => { 
     ...
     > 
       ...
       <div className="absolute inset-0 z-0 pointer-events-none"> 
         <AnimatePresence mode="popLayout"> 
           ...
           > 
             <img 
               src={optimizeImage(activeItem.imageSrc, 100, 30)} 
               alt="" 
               className={cn(
                 "h-full w-full object-cover saturate-200",
             ...
             return ( 
               <motion.div 
                 ...
                 animate={{ 
                   ...
                   rotateY: rotateY, 
                   opacity: opacity, 
                   filter: isMobile ? `brightness(${brightness})` : `blur(${blur}px) brightness(${brightness})`, 
                 }} 
                 transition={(val) => { 
               ...
               > 
                 <img 
                   src={optimizeImage(item.imageSrc, 400, 60)} 
                   alt={item.title} 
